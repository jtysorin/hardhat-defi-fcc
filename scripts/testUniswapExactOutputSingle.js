...
async function main() {
    ...
    const daiToGet = Math.floor(halfWethBalance.toString() * (1 / daiPrice.toNumber()));
    const daiToGetWei = ethers.utils.parseUnits(daiToGet.toString(), 18);
    console.log(`You want to get ${daiToGetWei} wei DAI from ${halfWethBalance} wei WETH`);
    ...
    const params = {
        ...
        amountOut: daiToGetWei,
        amountInMaximum: halfWethBalance,
        sqrtPriceLimitX96: 0,
    };
    ...
    console.log("Swap 2");
    await approveErc20(wethTokenAddress, swapRouter.address, halfWethBalance, deployer);
    const tx2 = await swapRouter.exactOutputSingle(params);
    tx2.wait(1);
    await getBalance(iWeth, iDai, deployer);
}
